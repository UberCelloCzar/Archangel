...
namespace Archangel
{
    ...
    // T 5/10/15- removed dash in favor of simpler one, added charge slash
    public class Player: Character
    {
        ...
        private int dashActive; // If player has dash status or not
        ...
        private Bullet whirlwind; // Energy arc generated by a charged slash
        private bool controlledFall = false;
        private int slashHold; // How long the slash key has been held
        private int whirlpoolTimer; // How long has the whirlpool been out
        private bool sPause;
            ...
            : base(X, Y, dir, spd, charSprite)
        {
            ...
            Texture2D[] slashSprite = new Texture2D[4]; // Pass in an array of the same sprite, to eliminate extra code
            slashSprite[0] = charSprite[17];
            slashSprite[1] = charSprite[17];
            slashSprite[2] = charSprite[17];
            slashSprite[3] = charSprite[17];
            whirlwind = new Bullet(-1000, -1000, 0, 10, 2, slashSprite); // Create the charge slash arc
        ...
        public override void Update() // Inputs and movement
        {
            ...
            if (onPlatform == false) // Sky update
            {
                ...
                else if (kstate.IsKeyUp(Keys.S) && slashHold >= 60)
                {
                    ...
                    {
                        case 0: // Right
                            whirlwind.spritePos = new Rectangle(spritePos.Right, spritePos.Top, whirlwind.spritePos.Width, whirlwind.spritePos.Height);
                            whirlwind.direction = 0;
                            break;
                        case 2: // Left
                            whirlwind.spritePos = new Rectangle(spritePos.Left - whirlwind.spritePos.Width, spritePos.Top, whirlwind.spritePos.Width, whirlwind.spritePos.Height);
                            whirlwind.direction = 1;
                            break;
                        case 4: // Up
                            whirlwind.spritePos = new Rectangle(spritePos.Left, spritePos.Top - whirlwind.spritePos.Height, whirlwind.spritePos.Width, whirlwind.spritePos.Height);
                            whirlwind.direction = 2;
                            break;
                        case 6: // Down
                            whirlwind.spritePos = new Rectangle(spritePos.Left, spritePos.Bottom, whirlwind.spritePos.Width, whirlwind.spritePos.Height);
                ...
                else if (kstate.IsKeyDown(Keys.S) && slashTime <= 0 && direction < 8 && slashHold == 0) // Wait for cooldown to end
                {
                    if (direction == 2 || direction == 3) // Pop up a hitbox in front of the character and put them in the appropriate slashing state
                    {
                        sBox = new Rectangle(spritePos.X, spritePos.Y, 40, 123);
                        direction = 10; // Left
                    }
                    else if (direction == 4 || direction == 5)
                    {
                        sBox = new Rectangle(spritePos.X, spritePos.Y, 114, 26);
                        direction = 11; // Up
                    }
                    else if (direction == 6 || direction == 7)
                    {
                        sBox = new Rectangle(spritePos.X, spritePos.Bottom - 26, 114, 26);
                        direction = 12; // Down
                    }
                    else
                    {
                        sBox = new Rectangle(spritePos.Right - 40, spritePos.Y, 40, 123);
                        direction = 9; // Right
                    }
                    slashHold = 1;
                    slashFrame = 0;
                    ...
                    else
                    {
                        switch (direction)
                        {
                            case 9: // Right
                                ...
                                {
                                    sBox = new Rectangle(spritePos.Right - 40, spritePos.Y, 40, 100);
                                }
                                else if (slashFrame > 12)
                                {
                                    sBox = new Rectangle(spritePos.Right - 40, spritePos.Y, 40, 114);
                                }
                                break;
                            case 10: // Left
                                if (slashFrame > 6 && slashFrame < 13)
                                {
                                    sBox = new Rectangle(spritePos.X, spritePos.Y, 40, 100);
                                }
                                else if (slashFrame > 12)
                                {
                ...
                if (this.spritePos.Intersects(platform.spritePos))
                {
                    switch (direction)
                    {
                        ...
                        default: // Catches both moveDown and falling
                            if (platform.spritePos.Right > spriteArray[14].Width && platform.spritePos.X < (Game1.clientWidth - spriteArray[14].Width))
                            {
                            ...
                            {
                                spritePos = new Rectangle(spritePos.X, platform.spritePos.Top - spritePos.Height, spritePos.Width, spritePos.Height); // Collides with top but not enough space to land
                            }
                            break;
                    }
                }
        ...
        public override void Draw(SpriteBatch spriteBatch) // Draw the character's sprite
        {
            switch (direction) // Draw the sprites
            {
                case 0: case 1:
                    ...
                    break;
                case 2: case 3:
                    spritePos = new Rectangle(spritePos.X, spritePos.Y, spriteArray[2].Width, spriteArray[2].Height);
                    spriteBatch.Draw(spriteArray[2], spritePos, color);// Left
                    break;
                case 4: case 5:
                ...
                case 9:
                    ...
                    {
                        spriteBatch.Draw(spriteArray[9], new Rectangle(spritePos.X, spritePos.Y, 139, 123), new Rectangle(0, 0, 139, 123), color);
                    }
                    else if (slashFrame > 6 && slashFrame < 13) // Frame 2
                    {
                        spriteBatch.Draw(spriteArray[9], new Rectangle(spritePos.X, spritePos.Y, 169, 100), new Rectangle(139, 0, 169, 100), color);
                    }
                    else if (slashFrame > 12) // Frame 3
                    {
                        spriteBatch.Draw(spriteArray[9], new Rectangle(spritePos.X, spritePos.Y, 155, 114), new Rectangle(308, 0, 155, 114), color);
                    }
                ...
                case 10:
                    if (slashFrame < 7) // Frame 1
                    {
                        spriteBatch.Draw(spriteArray[10], new Rectangle(spritePos.X, spritePos.Y, 139, 123), new Rectangle(0, 0, 139, 123), color);
                    }
                    else if (slashFrame > 6 && slashFrame < 13) // Frame 2
                    {
                        spriteBatch.Draw(spriteArray[10], new Rectangle(spritePos.X, spritePos.Y, 169, 100), new Rectangle(139, 0, 169, 100), color);
                    }
                    else if (slashFrame > 12) // Frame 3
                    {
                        spriteBatch.Draw(spriteArray[10], new Rectangle(spritePos.X, spritePos.Y, 149, 112), new Rectangle(308, 0, 149, 112), color);
                    }
                    slashFrame++; // Move to the next frame
                    break; // Left Slash
                case 11:
                ...
                case 12:
                    if (slashFrame < 7) // Frame 1
                    {
                        spriteBatch.Draw(spriteArray[12], new Rectangle(spritePos.X, spritePos.Y, 112, 107), new Rectangle(0, 0, 112, 107), color);
                    }
                    else if (slashFrame > 6 && slashFrame < 13) // Frame 2
                    {
